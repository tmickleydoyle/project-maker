...
export async function executeScript(script: string): Promise<void> {
    ...
    const bashScripts = script.match(/```bash\n([\s\S]*?)```/g)?.map(block => 
        block.replace(/```bash\n/, '').replace(/```$/, '')
    ) || [];
            ...
            while (!success && attempts < 3) {
                try {
                    await new Promise((resolve, reject) => {
                        console.log(`Executing ${scriptFile.path} (Attempt ${attempts + 1}/3)...`);
                        const process = exec(`./${scriptFile.path}`, { maxBuffer: 1024 * 1024 * 10 });
                    ...
                    if (attempts < 3) {
                        ...
                        const completion = await openai.chat.completions.create({
                            messages: [
                                {
                                    role: "system",
                                    content: "You are a bash script debugging assistant. Fix the script based on the error message."
                                },
                                {
                                    role: "user",
